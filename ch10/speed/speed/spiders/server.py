...
class BaseResource(Resource):
    ...
    def render_GET(self, request):
        # https://twistedmatrix.com/documents/13.0.0/web/howto/web-in-60/asynchronous-deferred.html
...
class Api(BaseResource):
    ...
    def __init__(self, settings):
        delay = settings.getfloat('SPEED_API_T_RESPONSE', settings.getfloat('SPEED_T_RESPONSE', 3))
...
class Index(BaseResource):
    ...
    def __init__(self, settings):
        delay = settings.getfloat('SPEED_INDEX_T_RESPONSE', settings.getfloat('SPEED_T_RESPONSE', 0.5))
        BaseResource.__init__(self, delay)
        self.details_per_index = settings.getint('SPEED_DETAILS_PER_INDEX_PAGE', 20)
    ...
    def _delayedRender(self, request):
        ...
        max_pages = (self.limit + page_worth - 1) / page_worth # = divide with roundup
        ...
        if p >= 1 and p <= max_pages:
            ...
            for i in xrange(self.details_per_index):
                ...
                if id <= self.limit:
                    irange = sorted(list(set([id, min(id+self.items_per_page-1, self.limit)])))
                    srange = "-".join([str(i) for i in irange])
                    request.write('<li><a class="item" href="../detail?id0=%d">item %s</a></li>' % (id, srange))
            
            request.write('</ul>')
            
            for i in xrange(self.index_lookahead):
                if (p+i) < max_pages:
                    request.write('<a class="nav" href="index?p=%d">next</a> ' % (p+i+1))
...
class Detail(BaseResource):
    ...
    def __init__(self, settings):
        delay = settings.getfloat('SPEED_DETAIL_T_RESPONSE', settings.getfloat('SPEED_T_RESPONSE', 1))
        BaseResource.__init__(self, delay)
        self.items_per_page = settings.getint('SPEED_ITEMS_PER_DETAIL', 1)
        self.limit = settings.getint('SPEED_TOTAL_ITEMS', 1000)
    ...
    def _delayedRender(self, request):
        ...
        for idx in xrange(id0, min(id0+self.items_per_page, self.limit+1)):
            ...
            request.write('<div class="info">useful info for id: %d</div>' % idx)
            request.write('</li>')
        request.write('</ul>')
        request.finish()
...
class Root(Resource):
    ...
    def render_GET(self, request):
        return 'Resource not found. Try: <a href="api?text=foo">api</a>, <a href="index?p=1">index</a>, <a href="detail?id0=1">detail</a>'
